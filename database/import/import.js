...
async function loadFiles({
  ...
  const mainTablesSql = await fsp.readFile(
    path.resolve(__dirname, "../schema/tables/main.sql"),
    "utf-8",
  );
  const mainIndexesSql = await fsp.readFile(
    path.resolve(__dirname, "../schema/indexes/main.sql"),
    "utf-8",
  );
  const log = (message) => console.log(`[${getTimestamp()}] ${message}`);

  const geneExpressionTablesSql = await fsp.readFile(
    path.resolve(__dirname, "../schema/tables/gene_expression.sql"),
    "utf-8",
  );
  ...
  for (const {
    ...
    insertStmt,
    insertTypeStmt,
    getInsertGeneExpressionStmt,
    getInsertGeneCountStmt,
    tablePrefix,
  } of files) {
    const csvStream = fs
    ...
    for await (const line of csvStream) {
      ...
      if (geneValuesBuffer.__size > bufferSize) {
        log(
          `Inserting ${bufferSize * genes.length} buffered records across ${
            genes.length
          } tables`,
        );
      ...
      if (id % 1e2 === 0) log(`Inserted ${id} records into ${tablePrefix}`);
    ...
    for (let gene of genes) getInsertGeneCountStmt({ gene }).run();
